    ...
    <div className="w-full min-w-0 max-w-4xl mx-auto space-y-10">
      ...
      <section className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm">
        ...
        <form action={saveSettings} className="space-y-6">
          ...
          <button type="submit" className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-colors text-sm shadow-sm">
      ...
      <section className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 shadow-sm">
        ...
        <div className="space-y-4 mb-8">
          {sources.map(source => (
            <div key={source.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-neutral-50 dark:bg-neutral-950 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-medium text-neutral-900 dark:text-white break-words">{source.name}</span>
                  <span className="text-xs px-2 py-0.5 bg-neutral-200 dark:bg-neutral-800 rounded-full text-neutral-600 dark:text-neutral-400 whitespace-nowrap">{source.category}</span>
                </div>
                <a href={source.url} target="_blank" rel="noopener noreferrer" className="block text-sm text-neutral-500 mt-1 hover:text-indigo-600 hover:underline truncate">{source.url}</a>
              </div>
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-800 pt-6 mt-6">
          ...
          <form action={addSource} className="flex flex-col sm:flex-row gap-4">
            ...
            <button type="submit" className="w-full sm:w-auto bg-neutral-800 hover:bg-neutral-900 dark:bg-neutral-200 dark:hover:bg-white text-white dark:text-neutral-900 px-4 py-2.5 rounded-lg font-medium transition-colors text-sm whitespace-nowrap shadow-sm">
              Ajouter
            </button>
          </form>
        </div>
